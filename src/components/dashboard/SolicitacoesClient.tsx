  ...
  const filtradas = useMemo(() => {
    return solicitacoes.filter(s => {
      ...
      const matchBusca =
        ...
        s.dados_ppp?.trabalhador_nome?.toLowerCase().includes(termo);
  ...
  return (
    <div className="p-6 space-y-6">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50">
                ...
                <th className="text-left px-5 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">Trabalhador</th>
                <th className="text-left px-5 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">Status</th>
                <th className="text-left px-5 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">Responsável</th>
                <th className="text-left px-5 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">Recebido em</th>
              ...
              {filtradas.map(s => (
                <tr key={s.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-5 py-3.5 text-slate-700">
                    {s.dados_ppp?.trabalhador_nome || <span className="text-slate-300">Não informado</span>}
                  </td>
                  <td className="px-5 py-3.5">
                    <span className={cn(
                  ...
                  <td className="px-5 py-3.5">
                    <Link
                      ...
                      className="text-navy font-medium text-xs hover:underline"
                    >
                      Ver →
                    </Link>
                  </td>
      ...
      <p className="text-xs text-slate-400 text-right">
        {filtradas.length} de {solicitacoes.length} solicitações
      </p>
    </div>
  );
}
